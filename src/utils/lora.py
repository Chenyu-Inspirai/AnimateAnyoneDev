...
def inject_trainable_lora_extended(
    ...
    if loras != None:
        loras = torch.load(loras, map_location='cuda:0')
    if True:
        for target_replace_module_i in target_replace_module:
            for _module, name, _child_module in _find_modules(
                ...
                if issubclass(_child_module.__class__, nn.Linear):
                    weight = _child_module.weight
                    bias = _child_module.bias
                    _tmp = LoraInjectedLinear(
                        _child_module.in_features,
                    ...
                    if bias is not None:
                        _tmp.linear.bias = bias
                elif issubclass(_child_module.__class__, nn.Conv2d):
                    weight = _child_module.weight
                    bias = _child_module.bias
                ...
                elif issubclass(_child_module.__class__, nn.Conv3d):
                    weight = _child_module.weight
                    bias = _child_module.bias
                ...
                if loras != None:
                    _module._modules[name].lora_up.weight = loras.pop(0).cuda()
                    _module._modules[name].lora_down.weight = loras.pop(0).cuda()
