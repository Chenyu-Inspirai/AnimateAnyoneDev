...
def main(cfg):
    ...
    # denoising_unet_lora_config = LoraConfig(
    #     r=cfg.lora_rank,
    #     lora_alpha=cfg.lora_alpha,
    #     init_lora_weights="gaussian",
    #     target_modules=["to_k", "to_q", "to_v", "to_out.0"],
    ...
    reference_unet.add_adapter(ref_unet_lora_config)
    # denoising_unet.add_adapter(denoising_unet_lora_config)
    logger.info("Loras initalized")
    cast_training_params(reference_unet, dtype=torch.float32)
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        if (
            ...
            reference_lora_state_dict =  convert_state_dict_to_diffusers(get_peft_model_state_dict(unwrap_net.reference_unet))
            # denoising_lora_state_dict =  convert_state_dict_to_diffusers(get_peft_model_state_dict(unwrap_net.denoising_unet))
            
            
            StableDiffusionPipeline.save_lora_weights(
            save_directory=f"{save_dir}/reference_lora-{global_step}",
            unet_lora_layers=reference_lora_state_dict,
            safe_serialization=True
            )
            
            # StableDiffusionPipeline.save_lora_weights(
            # save_directory=f"{save_dir}/denoising_lora-{global_step}",
            # unet_lora_layers=denoising_lora_state_dict,
            # safe_serialization=True
            # )
